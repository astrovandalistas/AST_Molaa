...
class AST(PrototypeInterface):
    ...
    def setup(self):
        ...
        self.currentState = AST.STATE_WAIT
        self.lastOnTime = time.time()
        self.lastOffTime = time.time()
        self._setupGpio()
    def loop(self):
        ## check state
        if ((self.currentState == AST.STATE_WAIT) and
            (time.time()-self.lastOffTime > AST.MOTOR_OFF_TIME) and
            (not self.messageQ.empty())):
            (locale,type,txt) = self.messageQ.get()
            ## turn gpio(s) on
            for m in AST.MOTOR_PINS:
                self.gpio.digitalWrite(m,self.gpio.HIGH)
            self.currentState = AST.STATE_BANG
            self.lastOnTime = time.time()
        elif ((self.currentState == AST.STATE_BANG) and
              (time.time()-self.lastOnTime > AST.MOTOR_ON_TIME)):
            ## turn off gpio(s)
            for m in AST.MOTOR_PINS:
                self.gpio.digitalWrite(m,self.gpio.LOW)
            self.currentState = AST.STATE_WAIT
...
if __name__=="__main__":
    ...
    mAST = AST(inIp, inPort,localNetAddress,localNetPort)
    runPrototype(mAST)
